...
#define PRESENTATION zathura->modes.presentation
  ...
  bool_value = false;
  girara_setting_add(gsession, "search-first-only",          &bool_value,  BOOLEAN, false, _("Only match the first found search result"), NULL, NULL);
  bool_value = true;
...
void config_load_files(zathura_t* zathura) {
  ...
  if (config_path != NULL && config_path[0] != '\0') {
    g_auto(GStrv) config_dirs = g_strsplit(config_path, ":", 0);
